...
> [!WARNING]
> Внимание!!! Существуют розетки `_TZ3000_gjnozsaz`, которые имеют другой `image type` и `manufacturer code`. Анализ выяснил, что у них немного другой чип, точнее чип тот же, но флеш-памяти в два раза меньше. Они не обновляются. И не нужно пытаться их обновить, получите кирпич с вероятностью 146%. Пока побороть их не получается.
